...
public class ImageMessageContentViewHolder extends MediaMessageContentViewHolder {
    ...
    public void onBind(UiMessage message) {
        ...
        if (!TextUtils.isEmpty(imageMessage.localPath)) {
            ...
            if (thumbnail != null) {
                request = (GlideRequest<Drawable>) request.placeholder(new BitmapDrawable(fragment.getResources(), imageMessage.getThumbnail()));
            } else {
                request = (GlideRequest<Drawable>) request.placeholder(R.mipmap.img_error);
            }
            request.centerCrop()
                    .into(imageView);
        }
