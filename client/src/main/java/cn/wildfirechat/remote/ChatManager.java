...
//    }
    ...
    public void sendMessage(final Message msg, final int expireDuration, final SendMessageCallback callback) {
        ...
        boolean canChat = sp.getBoolean("canChat", true);
        if (!canChat) {
            Toast.makeText(gContext, "can not to chat", Toast.LENGTH_SHORT).show();
            for (OnSendMessageListener listener : sendMessageListeners) {
                listener.onSendFail(msg, 500);
            }
            return;
        }
        try {
...
//        UserInfo userInfo = getUserInfo(memberId, groupId, false);
    ...
    public String getUserDisplayName(String userId, UserInfoCallBack callback) {

        UserInfo userInfo = getUserInfo(userId, false, callback);
    ...
    public UserInfo getUserInfo(String userId, boolean refresh, UserInfoCallBack callback) {
        return getUserInfo(userId, null, refresh, callback);
    }
     ...
     * @return
     */
    public UserInfo getUserInfo(String userId, String groupId, boolean refresh, UserInfoCallBack callback) {
...
//        }
        if (userSource != null) {
            userInfo = userSource.getUser(userId, callback);
            if (userInfo == null) {
     ...
     * @return
     */
    public List<UserInfo> getUserInfos(List<String> userIds, String groupId, UserInfoCallBack callback) {
        ...
        if (userSource != null) {
            ...
            for (String userId : userIds) {
                userInfos.add(userSource.getUser(userId, callback));
            }
            return userInfos;
        }
